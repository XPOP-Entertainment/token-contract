...
contract Lockable is Context {
  ...
  /**
    ...
    */
  function _addInvestorLock(address account, uint amount, uint startsAt, uint period, uint count) internal {
    ...
    require(count > 1, "Investor Lock: count is below 1");
  ...
  function _removeInvestorLock(address account) internal {
    ...
    */
  function getInvestorLock(address account) public view returns (uint, uint, uint, uint){
  ...
  function getInvestorLockedAmount(address account) public view returns (uint) {
    ...
    if (amount > 0) {
      ...
      uint period = _investorLocks[account].period;
      uint count = _investorLocks[account].count;
      uint expiresAt = startsAt.add(period.mul(count.sub(1)));
      uint timestamp = block.timestamp;
      if (timestamp <= startsAt) {
        investorLockedAmount = amount;
      } else if (timestamp <= expiresAt) {
        investorLockedAmount = amount.mul(expiresAt.sub(timestamp).div(period).add(1)).div(count);
      }
    }
...
contract XPOP is Pausable, Ownable, Burnable, Lockable, ERC20 {
  ...
  function removeTimeLock(address account, uint8 index) public onlyOwner whenNotPaused {
    ...
    */
  function addInvestorLock(address account, uint startsAt, uint period, uint count) public onlyLocker whenNotPaused {
